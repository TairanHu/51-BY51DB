...
{
	if(NRF24L01_Check())Show_f8x16str(0,0,"NRF CheckFail");
	else Show_f8x16str(0,0,"NRF CheckOk");
}
...
uchar NRF24L01_Check(void)
{
	...
	uchar i;
	//SPI1_SetSpeed(SPI_SPEED_4); //spi速度为9Mhz（24L01的最大SPI时钟为10Mhz）   	 
...
{
	NRF24L01_CE=0;	  
  	NRF24L01_Write_Buf(WRITE_REG+RX_ADDR_P0,(uchar*)RX_ADDRESS,RX_ADR_WIDTH);//写RX节点地址
  	...
  	NRF24L01_Write_Reg(WRITE_REG+CONFIG1, 0x0f);//配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式 
...
{														 
	NRF24L01_CE=0;	    
  	...
  	NRF24L01_Write_Reg(WRITE_REG+RF_SETUP,0x0f);  //设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
  	NRF24L01_Write_Reg(WRITE_REG+CONFIG1,0x0e);    //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式,开启所有中断
